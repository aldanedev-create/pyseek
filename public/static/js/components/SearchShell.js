...
const __component = {
  ...
  methods: {
    ...
    async suggest(value) {
      const text = typeof value === 'string' ? value : (value?.target?.value || ''); if (text.length < 2) { this.suggestions = []; return }; try { const response = await fetch('/api/suggest?q=' + encodeURIComponent(text)); this.suggestions = (await response.json()).suggestions || [] } catch (_) { this.suggestions = [] }
    },
    async loadHistory() {
