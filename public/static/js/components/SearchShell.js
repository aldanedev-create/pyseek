...
const __component = {
  ...
  data() {
    return { query: '', results: [], history: [], suggestions: [], stats: { documents: 0, queued: 0 }, loading: false, error: '', searched: false };
  },
  ...
  methods: {
    ...
    async loadStats() {
      try { const response = await fetch('/api/stats'); if (response.ok) this.stats = (await response.json()) } catch (_) {}
    },
    async suggest(value) {
      const text = typeof value === 'string' ? value : (value?.target?.value || ''); this.query = String(text); if (text.length < 2) { this.suggestions = []; return }; try { const response = await fetch('/api/suggest?q=' + encodeURIComponent(text)); this.suggestions = (await response.json()).suggestions || [] } catch (_) { this.suggestions = [] }
    },
    async loadHistory() {
      if (window.PySeekStorage) this.history = await window.PySeekStorage.list('history')
    },
    async remember(value) {
      if (window.PySeekStorage) { await window.PySeekStorage.put('history', { id: value, value: value, createdAt: Date.now() }); this.loadHistory() }
    },
    async clearHistory() {
      if (window.PySeekStorage) { await window.PySeekStorage.clear('history'); this.history = [] }
    },
    chooseSuggestion(value) {
      this.suggestions = []; this.search(typeof value === 'string' ? value : value?.target?.value)
    },
  },

  mounted() {
    this.loadHistory(); this.loadStats()
  },

  template: `<div class="shell"><Header data-teloce-bind-documents="stats.documents"></Header><section class="hero"><p class="eyebrow">OPEN WEB INDEX · POWERED BY FLAXON</p><h1>Find the useful web.</h1><p class="lede">A focused search engine built with Python, PostgreSQL, and composable <code>.vel</code>components.</p><SearchBar data-teloce-bind-query="query" data-teloce-bind-suggestions="suggestions" data-teloce-bind-loading="loading" @search="search" @input="suggest" @choose="chooseSuggestion"></SearchBar></section><section class="layout"><div class="results-panel"><if condition="loading"><div class="notice">Searching the PySeek index…</div></if><if condition="error"><div class="notice error">{{ error }}</div></if><if condition="searched && !results.length && !loading"><div class="notice">No indexed pages matched. Seed a site and let the crawler build the index.</div></if><SearchResults data-teloce-bind-results="results" data-teloce-bind-query="query"></SearchResults></div><aside><CrawlStatus data-teloce-bind-stats="stats" @refresh="loadStats"></CrawlStatus><SearchHistory data-teloce-bind-items="history" @pick="search" @clear="clearHistory"></SearchHistory></aside></section><footer><span>PySeek is an experimental, transparent index—not a replacement for the whole web.</span><span>Built with Teloce-Py + Flaxon</span></footer></div>`,
};

const __components = {"Header": Header, "SearchBar": SearchBar, "SearchResults": SearchResults, "SearchHistory": SearchHistory, "CrawlStatus": CrawlStatus};
...
const __template = "<div class=\"shell\"><Header data-teloce-bind-documents=\"stats.documents\"></Header><section class=\"hero\"><p class=\"eyebrow\">OPEN WEB INDEX · POWERED BY FLAXON</p><h1>Find the useful web.</h1><p class=\"lede\">A focused search engine built with Python, PostgreSQL, and composable <code>.vel</code>components.</p><SearchBar data-teloce-bind-query=\"query\" data-teloce-bind-suggestions=\"suggestions\" data-teloce-bind-loading=\"loading\" @search=\"search\" @input=\"suggest\" @choose=\"chooseSuggestion\"></SearchBar></section><section class=\"layout\"><div class=\"results-panel\"><if condition=\"loading\"><div class=\"notice\">Searching the PySeek index…</div></if><if condition=\"error\"><div class=\"notice error\">{{ error }}</div></if><if condition=\"searched && !results.length && !loading\"><div class=\"notice\">No indexed pages matched. Seed a site and let the crawler build the index.</div></if><SearchResults data-teloce-bind-results=\"results\" data-teloce-bind-query=\"query\"></SearchResults></div><aside><CrawlStatus data-teloce-bind-stats=\"stats\" @refresh=\"loadStats\"></CrawlStatus><SearchHistory data-teloce-bind-items=\"history\" @pick=\"search\" @clear=\"clearHistory\"></SearchHistory></aside></section><footer><span>PySeek is an experimental, transparent index—not a replacement for the whole web.</span><span>Built with Teloce-Py + Flaxon</span></footer></div>";
const __style = "";
...
const __initialData = { query: '', results: [], history: [], suggestions: [], stats: { documents: 0, queued: 0 }, loading: false, error: '', searched: false };
...
const __installStyle = () => {
  ...
  const styleId = "teloce-style-b719ceed6";
