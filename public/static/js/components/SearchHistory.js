...
const __component = {
  ...
  props: {"items": {"type": "Array", "required": false, "default": null, "validator": null}},

  template: `<section class="side-card"><div class="side-heading"><h2>Recent searches</h2><if condition="items && items.length"><button class="text-button" @click="$emit('clear')">Clear</button></if></div><if condition="!items || !items.length"><p class="muted">Your searches stay in this browser.</p></if><for key="item.id" item="item" in="items"><button data-teloce-key="{{ item.item.id }}" class="history-item" @click="$emit('pick', item.value)">⌕ {{ item.value }}</button></for></section>`,
};
...
const __template = "<section class=\"side-card\"><div class=\"side-heading\"><h2>Recent searches</h2><if condition=\"items && items.length\"><button class=\"text-button\" @click=\"$emit('clear')\">Clear</button></if></div><if condition=\"!items || !items.length\"><p class=\"muted\">Your searches stay in this browser.</p></if><for key=\"item.id\" item=\"item\" in=\"items\"><button data-teloce-key=\"{{ item.item.id }}\" class=\"history-item\" @click=\"$emit('pick', item.value)\">⌕ {{ item.value }}</button></for></section>";
const __style = "";
...
const __installStyle = () => {
  ...
  const styleId = "teloce-style-ae22419d2";
