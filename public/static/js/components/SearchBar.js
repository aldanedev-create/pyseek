...
const __component = {
  ...
  template: `<div class="search-wrap"><form class="search-form" @submit="submit"><span class="search-icon" aria-hidden="true">⌕</span><input placeholder="Search the indexed web" aria-label="Search" autofocus="" data-teloce-model="localQuery"><button type="submit">Search</button></form><if condition="suggestions && suggestions.length"><div class="suggestions"><for key="item" item="item" in="suggestions"><button data-teloce-key="{{ item.item }}" type="button" @click="$emit('choose', item)">{{ item }}</button></for></div></if></div>`,
};
...
const __template = "<div class=\"search-wrap\"><form class=\"search-form\" @submit=\"submit\"><span class=\"search-icon\" aria-hidden=\"true\">⌕</span><input placeholder=\"Search the indexed web\" aria-label=\"Search\" autofocus=\"\" data-teloce-model=\"localQuery\"><button type=\"submit\">Search</button></form><if condition=\"suggestions && suggestions.length\"><div class=\"suggestions\"><for key=\"item\" item=\"item\" in=\"suggestions\"><button data-teloce-key=\"{{ item.item }}\" type=\"button\" @click=\"$emit('choose', item)\">{{ item }}</button></for></div></if></div>";
const __style = "";
...
const __installStyle = () => {
  ...
  const styleId = "teloce-style-3de45a766";
